    ...
    exports.put = function (req, res) {
        if (!req.user) {
            ...
            mdb.State.update({
                ...
                user: req.user._id
            }, { $set: { data: req.body } }, { upsert: true }, function (err, affected, raw) {
                res.json({ ok: true });
            });
        }
    };
    ...
    exports.completion = function (req, res) {
        if (!req.user) {
            ...
            mdb.Completion.update({
                activityHash: req.params.activityHash,
                user: req.user._id
            }, { $set: { complete: req.body.complete, date: new Date() } }, { upsert: true }, function (err, affected, raw) {
                if (err) {
    ...
    exports.remove = function (req, res) {
        if (!req.user) {
            ...
            mdb.State.update({
                activityHash: req.params.activityHash,
                user: req.user._id
            }, { $set: { data: {} } }, { upsert: true }, function (err, affected, raw) {
                res.json({ ok: true });
            });
        }
    };
