        ...
        function (callback) {
            ...
            mdb.Blob.findOne({ hash: hash }, function (err, blob) {
                if (blob) {
                    ...
                    var new_blob = new mdb.Blob({
                        hash: hash,
                        data: data
                    });

                    new_blob.save(callback);
                }
            });
        }
    ], function (err, result) {
        ...
        if (err) {
            ...
            data.forEach(function (branch) {
                if (branch.commit.sha == sha) {
                    ...
                    mdb.Branch.findOneAndUpdate({
                        commit: b.commit,
                        name: b.name,
                        repository: b.repository,
                        owner: b.owner
                    }, b, { upsert: true }, function (err, doc) {
                        return;
                    });
                }
            });
    ...
    github.repos.getCommits({ user: owner, repo: repo, sha: sha }, function (err, data) {
        if (err) {
            ...
            if (data.length == 0) {
                ...
                mdb.Commit.findOneAndUpdate({ sha: commit.sha }, commit, { upsert: true }, function (err, doc) {
                    if (err) {
                        res.status(500).send();
                    } else {
                        res.status(200).send();
                        mdb.User.update({ _id: req.user._id }, { $addToSet: { instructor: commit.sha } }, function (err, doc) {});
                    }
                });
            }
        }
...
exports.putBareCommit = function (req, res) {
    ...
    mdb.Commit.findOneAndUpdate({ sha: commit.sha }, commit, { upsert: true }, function (err, doc) {
        if (err) {
            ...
            res.status(200).send();
            mdb.User.update({ _id: req.user._id }, { $addToSet: { instructor: sha } }, function (err, doc) {});
        }
    });
};
            ...
            if (branch) {
                mdb.Branch.find({ owner: branch.owner, repository: branch.repository }, function (err, branches) {
                    if (err) {
                        ...
                        callback(err, branches.map(function (branch) {
