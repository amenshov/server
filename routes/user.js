...
exports.getCurrent = function (req, res) {
    ...
    if (req.user.googleOpenId) {
        req.user.googleOpenId = 'token';
    }

    if (req.user.courseraOAuthId) {
        req.user.courseraOAuthId = 'token';
    }

    if (req.user.githubId) {
        req.user.githubId = 'token';
    }

    if (req.user.twitterOAuthId) {
        req.user.twitterOAuthId = 'token';
    }
...
exports.update = function (req, res) {
    ...
    mdb.User.findOne({ _id: new mongo.ObjectID(id) }, function (err, document) {
        ...
        mdb.User.update({ _id: new mongo.ObjectID(id) }, { $set: hash }, function (err, d) {
            if (err) {
                res.send(500);
            } else {
                res.render('user/profile', {
    ...
    mdb.User.findOne({ _id: new mongo.ObjectID(id) }, function (err, instructor) {
        ...
        github.user.get({}, function (err, githubUser) {
            ...
            github.repos.getCollaborator({
                ...
                mdb.Branch.find({ owner: owner, repository: repo }, { commit: 1, _id: 0 }, function (err, branches) {
                    if (err) {
                        res.status(500).send(err);
                    ...
                    var commits = branches.map(function (branch) {
                    ...
                    mdb.User.update({ _id: instructor._id }, { $set: hash }, function (err, document) {
                        if (err) {
                            res.status(500).send(err);
                        } else {
